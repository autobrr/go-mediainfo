...
- HEVC shared fix: `parseHEVCConfig` now always reports `Format_Tier` (Main as well as High) and gates `ChromaSubsampling_Position` on the SPS `chroma_loc_info_present_flag` (value from `chroma_sample_loc_type_top_field`, plumbed through `parseHEVCVUI`) instead of a hardcoded "Type 2" for 4:2:0 — also fixes MKV HEVC (`only matroska.go` calls `parseHEVCConfig`; TS/BDAV use the Annex B path).
