...
- Verified vs official v23.04 on root@media: 10-file real mkv set (9-file sweep + Takopii ep2) all at text diff `0` except Adjustment Bureau (`Errors: Missing ID_END` only); JSON matches official except pre-existing `Encoded_Application_Name/Version` split (newer-MediaInfo behavior we keep) and env-only `File_Created_Date`. TS/mp4 outputs byte-identical to main.
- Known remaining: `Errors: Missing ID_END` needs a real AAC `raw_data_block` element walker (huffman sections) to detect the missing `ID_END` terminator — not worth it yet; `Count of elements` on tiny synthetic text tracks; TS controls have drifted vs official on main (pre-existing, unrelated to Matroska work; e.g. `Nickelodeon Generic` default-text diff `41` for both main and this branch).
