...
- Known remaining (measured, not yet fixed): bit rate unit and precision rule (official uses `b/s` below 10 000, one decimal `kb/s` to 99 999, integer `kb/s` up to 10 000 kb/s, then one decimal `Mb/s`); E-AC-3 with `Format_AdditionalFeatures=Dep` shows as `AC-3` in text; `Maximum bit rate`, PGS `Codec ID/Info`, and FLAC `Frame rate`/`MD5` missing from text; display aspect ratio not snapped to standard cinema ratios; `Bits/(Pixel*Frame)` truncated instead of rounded; `Format tier` and chroma position not folded into their parent lines; `MaxCLL`/`MaxFALL` missing the `cd/m2` unit; `HDR_Format_Compression` emits a dangling `"None / "`. The same AC-3 text leak still exists on the mp4 and TS paths (mp4 is 5 lines on `mp4-01`); the MPEG-PS/DVD path keeps these lines on purpose.
