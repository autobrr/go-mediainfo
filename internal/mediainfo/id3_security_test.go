...
func TestParseID3RejectsTruncatedMaximumTagBeforeAllocation(t *testing.T) {
	header := []byte{'I', 'D', '3', 0x04, 0x00, 0x00, 0x7F, 0x7F, 0x7F, 0x7F}
	reader := bytes.NewReader(header)
	id3, ok := parseID3v2(reader, int64(len(header)), &embeddedAssetBudget{})
	if !ok {
...
func TestParseID3RejectsOverBudgetTagBeforeRead(t *testing.T) {
	...
	reader := bytes.NewReader(header)
	id3, ok := parseID3v2(reader, int64(10+tagSize), &embeddedAssetBudget{})
